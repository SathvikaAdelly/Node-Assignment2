...
app.post("/register/", async (request, response) => {
    const { usernsme. password, gender, name } = request.body;
...
app.post("/login/", async (request, response) => {
    ...
    if (dbUser !== undefined) {
        ...
        if (isPasswordMatch) {
            let jwtToken = jwtt.sign(username, "MY_SECRET_KEY");
...
app.get(
    ...
    async (request, response) => {
        ...
        const { likes } = await db.get(`select count(like_id) as likes from like where tweet_id = ${tweetID};`);
...
app.get(
    ...
    async (request, response) => {
        const { tweetId } = request.params;
        const replies = await db.all(`select user.name, reply.reply from reply natural join user 
